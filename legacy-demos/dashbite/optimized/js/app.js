/**
 * DashBite - Application Controller & User Flow (Baseline 2024 Optimized Edition)
 * Features progressive enhancement, cross-engine safety, and native Baseline APIs.
 */
...
function renderCategoryCarousel() {
  ...
  container.innerHTML = CATEGORIES.map(cat => `
    <button class="category-pill-item ${state.activeCategory === cat.id ? 'active' : ''}" data-category="${cat.id}" aria-label="Filter by ${cat.name}">
      <div class="category-icon-wrap" aria-hidden="true">${cat.icon}</div>
...
/**
 * Filter & Render Restaurant Feed with Subgrid-aligned Cards
 */
function renderRestaurantGrid() {
  ...
  grid.innerHTML = filtered.map(res => `
    <article class="restaurant-card" data-restaurant-id="${res.id}" tabindex="0" role="button" aria-label="${res.name}, ${res.cuisine}">
      <div class="card-media-wrapper">
        <img src="${res.image}" alt="${res.name}" class="card-restaurant-img" loading="lazy" onerror="this.onerror=null;this.src='assets/images/burger.jpg';">
        ${res.offerText ? `<span class="card-offer-badge">${res.offerText}</span>` : ''}
        <button class="card-favorite-btn" aria-label="Favorite ${res.name}" onclick="event.stopPropagation(); alert('Saved ${res.name} to favorites!')">❤️</button>
      </div>
      <div class="card-content">
        ...
        <div class="card-delivery-row">
          ...
          <div class="delivery-fee-wrap">
            <span class="delivery-fee-badge ${res.deliveryFee > 0 ? 'paid' : ''}" tabindex="0" role="tooltip" aria-describedby="tip-${res.id}">
              ${res.deliveryFee === 0 ? '$0 Delivery' : `$${res.deliveryFee.toFixed(2)} Fee`}
              <span class="fee-info-icon" aria-hidden="true">ℹ️</span>
            </span>
            <!-- Standard Baseline 2024 Tooltip with Relative Positioning & Zero JS dependency -->
            <div class="fee-tooltip" id="tip-${res.id}" role="tooltip">
              <strong>${res.name} Delivery Policy</strong><br>
              Standard courier dispatch fee. Free with DashPass.
            </div>
          </div>
        </div>
        ${res.isDashPass ? `
  ...
  grid.querySelectorAll('.restaurant-card').forEach(card => {
    ...
    card.addEventListener('keydown', (e) => {
      if (e.key === 'Enter' || e.key === ' ') {
        e.preventDefault();
        navigateToRestaurant(card.dataset.restaurantId);
      }
    });
  });
}
...
/**
 * Baseline 2024 Progressive Feature Detection: View Transitions API
 * Gracefully navigates on Safari <18, Firefox, and in-app webviews without JS exceptions.
 */
...
function renderRestaurantDetail(restaurantId) {
  ...
  container.innerHTML = `
    <button class="back-to-feed-btn" id="backToFeedBtn" aria-label="Back to restaurant feed">← Back to All Stores</button>
    <div class="restaurant-header-banner">
      <img src="${res.heroImage}" alt="${res.name}" class="res-banner-img" fetchpriority="high" onerror="this.onerror=null;this.src='assets/images/hero-banner.jpg';">
    ...
    <div class="menu-section">
      ...
      <div class="menu-items-grid">
        ${res.menu.map(item => `
          <div class="menu-item-card" data-item-id="${item.id}" tabindex="0" role="button" aria-label="Customize ${item.name}, $${item.basePrice.toFixed(2)}">
            ...
            <div class="item-visual-wrap">
              <img src="${item.image}" alt="${item.name}" class="item-img" loading="lazy" onerror="this.onerror=null;this.src='assets/images/burger.jpg';">
              <div class="item-add-btn-icon" aria-hidden="true">+</div>
            </div>
          </div>
        `).join('')}
      </div>
...
/**
 * Open Dish Customization Popover Dialog (Native Popover API Baseline 2024)
 */
...
function renderCustomizationContent(popover) {
  ...
  popover.innerHTML = `
    <div class="popover-header-media">
      ...
      <button class="popover-close-btn" id="closePopoverBtn" aria-label="Close dialog">&times;</button>
    </div>
    ...
    <div class="popover-footer-sticky">
      <div class="qty-counter-ctrl">
        <button class="qty-btn" id="qtyMinusBtn" aria-label="Decrease quantity">−</button>
        <span class="qty-number" id="qtyDisplay">1</span>
        <button class="qty-btn" id="qtyPlusBtn" aria-label="Increase quantity">+</button>
      </div>
      ...
      <div class="cart-item-details">
        ...
        <div class="cart-item-price-row">
          ...
          <div class="qty-counter-ctrl" style="padding: 2px 6px;">
            <button class="qty-btn" aria-label="Decrease quantity" onclick="window.updateCartItemQty('${item.cartItemId}', -1)">−</button>
            <span class="qty-number" style="font-size: 13px;">${item.quantity}</span>
            <button class="qty-btn" aria-label="Increase quantity" onclick="window.updateCartItemQty('${item.cartItemId}', 1)">+</button>
          </div>
        </div>
      </div>
    </div>
